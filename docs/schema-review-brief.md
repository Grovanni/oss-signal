...
For example, adding `localization_catalog_change`, `dominant_database_change`, `source_wording_change` or `container_image_update` does not require a new schema version because signal IDs are data values, not a closed enum in the JSON Schema.
